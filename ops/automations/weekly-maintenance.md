...
1. 健康检查：MCP 在线、存储容量、canary 召回验证。
   🔴 canary 召回 CLI-first（2026-09-04，v0.3.21，与每日哨兵一致）：自动化调度
   会话中 MCP 直连恒不可用（实测 direct=absent），首选
   `bash ~/.leafmem/leafmem-cli.sh recall "<canary 词>" 400`（HTTP 通道，launchd
   守护，hits>0 即判正常）；mcp__leafmem__memory_recall 恰在函数表时可顺带直调，
   absent 不判异常；CLI 也不可用 → 查 launchd 服务后降级宿主会话搜索。
