...
- **Automation active-loading channel (v0.3.21)**: in automation/scheduled sessions the host may fail to expose the \`mcp__leafmem__*\` tools even when the leafmem MCP server connects (known host regression: defer_loading:false ignored + deferred-index drop, 2026-09-04). Every automation that reads or writes LeafMem MUST use this downgrade chain, in order: ① direct \`mcp__leafmem__memory_recall\`/\`memory_write\` call; ② \`bash ~/.leafmem/leafmem-cli.sh recall|remember|...\` (HTTP channel to the launchd-managed agent service, independent of host MCP registration); ③ host conversation search (read-only fallback). When ① fails, do NOT silently treat it as a normal environment difference — record the failure and, if possible, grep the session log for "Indexing deferred tools for server: leafmem" as regression evidence.
