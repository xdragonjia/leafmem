...
- **Automation sessions: CLI-first channel (v0.3.21)**: in automation/scheduled sessions the host does NOT expose the \`mcp__leafmem__*\` tools (verified by a one-off automation on 2026-09-04: direct call returns absent on both 5.5.1 and 5.5.3 — the tools are forced into the deferred index and never resolved). Therefore every automation that reads or writes LeafMem MUST use the HTTP CLI as its PRIMARY channel, not as a fallback: ① \`bash ~/.leafmem/leafmem-cli.sh recall|remember|get|update|delete|list|stats|scopes|task-detail|commit-summary\` (talks to the launchd-managed agent service at 127.0.0.1:3377, independent of host MCP registration; installed/upgraded alongside the hooks); ② if the CLI itself is unreachable, check the launchd service (\`launchctl list | grep leafmem\`) and \`~/.leafmem/agent-service.json\`; ③ only then fall back to the host's conversation search (read-only). If the \`mcp__leafmem__*\` tools happen to be present in a session, a direct call is acceptable as a convenience, but their absence is expected and must NOT be treated as an error or silently retried via ToolSearch. Regression forensics (optional): grep the session log for "Indexing deferred tools for server: leafmem".
