...
**双宿主日常使用只有一个正确 scope：`agent:<宿主>`。**

- **共用拓扑**（推荐）：两宿主统一落 `agent:workbuddy`，互相可见可写——这是"共用一套记忆"的实现方式，而不是写 `user:` scope；
...
> 控制台「范围」选择器是动态的：默认「全部记忆」（不过滤），并列出实际有内容的 scope。正常运行时这里应该**只有 `agent:workbuddy`**；如果出现其他 scope，说明有历史错误记录需要修复（运维哨兵会检查并告警）。
